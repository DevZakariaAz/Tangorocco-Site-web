...
if (isset($_GET['publicationid'])) {
    ...
    $query = "
    SELECT p.titre, p.description, p.coverimage, AVG(r.rating) as average_rating
    FROM publication p
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_review'])) {
    ...
    if ($rating >= 1 && $rating <= 5 && !empty($comment)) {
        $QueryInsertReview = "INSERT INTO review (rating, comment, userid, publicationid) VALUES (:rating, :comment, :userid, :publicationid)";
        
        $insertStmt = $conn->prepare($QueryInsertReview);
...
<!--===========  Include footer from folder package  ===========-->
